...
В корне репозитория лежит файл **`kp-1os-edo-legasi.pdf`** — текстовое коммерческое предложение в формате A4 для передачи заказчику и печати. Кнопки «Скачать PDF» на лендинге ведут на этот файл.

Чтобы пересобрать PDF после правок текста, установите зависимости и запустите скрипт (нужны шрифты DejaVu, как в типичной установке Linux):

```bash
pip install -r requirements-dev.txt
python3 scripts/generate-kp-pdf.py
```
